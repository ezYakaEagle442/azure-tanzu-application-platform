...
az deployment group create --name storage -f iac/bicep/modules/tap/client-vm.bicep -g $RgName \
-p appName=petclitap \
-p location=westeurope \
-p windowsVMName=vm-winpetclinic \
-p adminUsername=adm_tap \
-p adminPassword=xxx \
...
param vnetName string = 'vnet-tap'
...
param windowsVMName string = 'vm-wintappetcli'
...
param nsgName string = 'nsg-tap-${appName}-app-client'
...
param nicName string = 'nic-tap-${appName}-client-vm'
...
win_vm_admin_username="adm_tap"
win_vm_admin_pwd="XXX" # The password length must be between 12 and 123. 
rg_name="rg-iac-tap-mic-srv"
vnet_name="vnet-tap"
appSubnet="snet-tap"
nsg="vnet-snet-tap-nsg-${location}"
